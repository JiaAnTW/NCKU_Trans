...
function major_QAIndex(props) {
    ...
    const sponCard = () => {
        if (props.is_fetch) {
            ...
            return (
                <div style={{ width: '100vw', maxWidth: '100%' }}>{output}</div>
            );
    ...
    const sponSingleCard = (number, datas) => {
        ...
        return (
            <Card
                style={{ width: cardWidth, height: 'auto', maxWidth: '100%' }}
            >
                <Card.Body
                    style={{
                        width: '100vw',
    ...
    const sponManyCard = (numberRow, datas) => {
        ...
        if (row > 1)
            return (
                <Row style={{ paddingBottom: cardPadding, maxWidth: '100%' }}>
    ...
    return <div className="QAIndex">{sponCard()}</div>;
}
