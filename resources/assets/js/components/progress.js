...
class progress extends Component {
    ...
    increase() {
        ...
        const std =
            this.props.value == 'null' ? 100 : Number(this.props.value) + 0.09;
        if (percent > std) {
            clearTimeout(this.tm);
            if (
                this.state.prevValue === 'null' ||
                Number(this.state.prevValue) > Number(this.props.value)
            )
                this.decrease();
            else
                this.setState({
        ...
        if (this.state.is_Mount == true) {
            ...
            if (
                ...
                if (
                    ...
                    Number(prevProps.value) > Number(this.props.value)
                ) {
                    this.decrease();
                } else this.increase();
            }
    ...
    render() {
        return (
            ...
            >
                <Circle
                    style={{ width: '90px', margin: '0 11.25px' }}
                    percent={this.state.percent.toFixed(1).toString()}
                    strokeWidth="4"
                    strokeColor="#E5446D"
                />
                <span
