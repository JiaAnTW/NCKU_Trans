...
class index extends Component {
    ...
    fliter(name) {
        if (name === 'none') return this.props.datas;
        else {
            var output = [];
            this.props.datas.forEach((element) => {
                if (element['department'] === name) output.push(element);
            });
            return output;
        }
    }
    ...
    changeRowCard() {
        if (window.innerWidth > 1400) this.setState({ row: 5 });
        else if (window.innerWidth >= 1140) this.setState({ row: 4 });
        else if (window.innerWidth >= 870) this.setState({ row: 3 });
        else if (window.innerWidth >= 596) this.setState({ row: 2 });
        else this.setState({ row: 1 });
    }
    ...
    sponSingleCard(number, datas) {
        var comment =
            datas[number]['comment'].length < 35
                ? datas[number]['comment']
                : datas[number]['comment'].substr(0, 34) + '  (...)';
        return (
            <Card style={{ width: '20rem', height: '20rem' }}>
                <Card.Body>
                    <Card.Title style={{ fontSize: '3.5rem' }}>
                        {'轉 ' + datas[number]['in_maj']}
                    </Card.Title>
