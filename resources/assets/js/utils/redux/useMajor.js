...
function useMajor() {
    ...
    useEffect(()=>{
        const data = majorData.filter((item)=>{
            return (item['in_maj'] === majorFilter['in_maj'] || majorFilter['in_maj']=== 'none') &&
                   (item['year'] === Number(majorFilter['year']) || majorFilter['year']=== 'none') &&
                   (item['department'] === majorFilter['department'] || majorFilter['department']=== 'none')
        });
        setDisplayData(data);
