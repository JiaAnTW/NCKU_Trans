...
import {Dropdown,InputGroup,DropdownButton,Button,Spinner} from 'react-bootstrap';
...
const content=()=>{
    if(this.state.is_fetch){
        return<div className="input-box"> 
            <input value={this.state.link} style={{height:"34px"}} onChange={this.changeData}/>
            <Button variant="light" onClick={this.handleClick} style={{borderRadius:"0"}}  disabled={this.state.is_send}>{(this.state.is_send)?<div><Spinner       as="span"
      animation="border"
      size="sm"
      role="status"
      aria-hidden="true" style={{width:"18px",height:"18px"}}/>傳送中</div>:"送出"}</Button>
        </div>
    }
}
    ...
    return (
      ...
      <div className="editStandard">
          <div style={{color:"white",fontSize:"16px",width:"350px",marginBottom:"20px",textAlign:"center"}}>放學校公布的轉系標準規定網址的地方</div>
          {content()}
      </div>
    );
  }
